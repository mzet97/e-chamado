...
export default function EditDepartmentPage() {
  ...
  useEffect(() => { departmentService.getById(id).then(res => { if (res.success) { setName(res.data.name); setDescription(res.data.description); } setLoading(false); }); }, [id]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); if (!name.trim()) return;
    setSaving(true);
    try { const res = await departmentService.update(id, { name, description }); if (res.success) router.push('/admin/departments'); } finally { setSaving(false); }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/departments" className="text-gray-500 hover:text-gray-700"><ArrowLeft size={20} /></Link>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Editar Departamento</h1>
      </div>
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6 space-y-5">
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
          <Link href="/admin/departments" className="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg">Cancelar</Link>
