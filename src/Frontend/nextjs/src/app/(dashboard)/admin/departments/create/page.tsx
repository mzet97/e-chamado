...
export default function CreateDepartmentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) return;
    setSaving(true);
    try { const res = await departmentService.create({ name, description }); if (res.success) router.push('/admin/departments'); } finally { setSaving(false); }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Novo Departamento</h1>
      </div>
