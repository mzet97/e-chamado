...
public class EntityEdgeCaseTests : UnitTestBase
{
    ...
    public void Entity_WithMaxDateTimeValues_ShouldHandleCorrectly()
    {
        ...
        var createdAtProperty = typeof(Category).BaseType!.GetProperty("CreatedAtUtc");

        // Assert - Verify normal behavior with extreme dates
        category.CreatedAtUtc.Should().BeCloseTo(DateTime.UtcNow, TimeSpan.FromSeconds(10));
        category.CreatedAtUtc.Should().BeAfter(DateTime.MinValue);
        category.CreatedAtUtc.Should().BeBefore(DateTime.MaxValue);
    }
